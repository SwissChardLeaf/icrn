...
class EndToEnd(unittest.TestCase):
    ...
    def test_winner_take_all(self):
        ...
        concs_spec = {
            X: 5.0 * img_batch,
            W: 100.0 * avg_img,
            XF: 2.0 * np.sum(100.0 * avg_img, axis=-1),
            SG: 100.0,
            RG: 100.0,
            A: 400.0,
            YF: 200.0,
        ...
        rate_data_spec = {alpha: 3.6e-3 * (np.ones((m, m)) - np.identity(m))}

        concs, rate_data, diff_data = wta_exp.dict_builder(
            concs_spec, rate_data_spec, {}, 9
        )
        sim_concs, _ = wta_exp.simulate_time(concs, rate_data, diff_data, 10.0)
    ...
    def test_hopfield(self):
        ...
        concs_spec = {Up: jnp.load(seed_path), Un: 0}
        ...
        diff_data_spec = {Up: jnp.load(Up_path), Un: jnp.load(Un_path)}

        init_concs, rate_data, diff_data = hf_exp.dict_builder(
