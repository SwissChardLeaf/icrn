...
from icrn import (
    ...
    ConcreteSpecies,
    Complex,
    ConcreteRateConstant,
    MassActionReaction,
    relu,
    NumericRateConstant,
    RateConstantFunction,
    FastReaction,
    ICRN,
...
class ConcreteSpeciesTests(unittest.TestCase):
    ...
    def test_eval(self):
        ...
        tensor_data = {A: jnp.arange(10).reshape((2, 5))}
...
class ComplexTests(unittest.TestCase):
    ...
    def test_index_symbols_replace(self):
        ...
        self.assertEqual(
            self.complex2.index_symbols_replace({i: 1, j: 2, k: 3}), target2
        )
...
class ConcreteRateConstantTests(unittest.TestCase):
    ...
    def test_eval(self):
        ...
        tensor_data = {alpha: jnp.arange(10).reshape((2, 5))}
...
class RateConstantFunctionTests(unittest.TestCase):
    ...
    def test_index_symbols_replace(self):
        ...
        self.assertEqual(
            ...
            RateConstantFunction(
                jax.nn.relu, RateConstantFunction(jnp.negative, alpha)
            ),
        )
        ...
        self.assertEqual(
            ...
            RateConstantFunction(
                jax.nn.relu, RateConstantFunction(jnp.negative, alpha[1, 1])
            ),
        )
        ...
        self.assertEqual(
            ...
            RateConstantFunction(
                jax.nn.relu, RateConstantFunction(jnp.negative, alpha[1, 2])
            ),
        )
    ...
    def test_eval(self):
        ...
        tensor_data = {alpha: jnp.array([[1, -2, 5], [3, -4, -6]])}
        ...
        self.assertTrue(
            jnp.all(self.add_one_alpha_12.eval(tensor_data) == jnp.array(-5))
        )
        ...
        self.assertTrue(
            jnp.all(self.relu_neg_alpha_12.eval(tensor_data) == jnp.array(6))
        )
    ...
    def test_einsum_index_symbol_string(self):
        ...
        self.assertEqual(
            self.add_one_alpha_ii.einsum_index_symbol_string(False, False), "ii"
        )
        self.assertEqual(
            self.add_one_alpha_ii.einsum_index_symbol_string(True, False), "ii"
        )
        self.assertEqual(
            self.add_one_alpha_ii.einsum_index_symbol_string(True, True), "HWii"
        )

        self.assertEqual(
            self.add_one_alpha_ij.einsum_index_symbol_string(False, False), "ij"
        )
        self.assertEqual(
            self.add_one_alpha_ij.einsum_index_symbol_string(True, False), "ij"
        )
        self.assertEqual(
            self.add_one_alpha_ij.einsum_index_symbol_string(True, True), "HWij"
        )

        self.assertEqual(
            self.add_one_alpha_12.einsum_index_symbol_string(False, False), ""
        )
        self.assertEqual(
            self.add_one_alpha_12.einsum_index_symbol_string(True, False), ""
        )
        self.assertEqual(
        ...
        self.assertEqual(
            self.relu_alpha_ii.einsum_index_symbol_string(True, False), "ii"
        )
        self.assertEqual(
            self.relu_alpha_ii.einsum_index_symbol_string(True, True), "HWii"
        )

        self.assertEqual(
            self.relu_alpha_ij.einsum_index_symbol_string(False, False), "ij"
        )
        self.assertEqual(
            self.relu_alpha_ij.einsum_index_symbol_string(True, False), "ij"
        )
        self.assertEqual(
            self.relu_alpha_ij.einsum_index_symbol_string(True, True), "HWij"
        )
...
class MassActionReactionTests(unittest.TestCase):
    def setUp(self):
        ...
        self.rxn1 = MassActionReaction(
            A[i, j] + 2 * B[j, k], A[i, j] + C[i, k], alpha[i]
        )
        self.rxn2 = MassActionReaction(D + E, F, relu(gamma[i, j]))
        self.rxn3 = MassActionReaction(
            D + B[i, j] + 3 * G[i], 2 * D + B[j, i] + G[k], 2.0
        )
    ...
    def test_index_symbols_replace(self):
        ...
        rxn1_target = MassActionReaction(
            A[1, 2] + 2 * B[2, 3], A[1, 2] + C[1, 3], alpha[1]
        )
        ...
        rxn3_target = MassActionReaction(
            D + B[1, 2] + 3 * G[1], 2 * D + B[2, 1] + G[3], 2.0
        )
...
class MichaelisMentenReactionTests(unittest.TestCase):
    def setUp(self):
        ...
        self.rxn1 = MichaelisMentenReaction(S, E, P, k, 1.0)
        self.rxn2 = MichaelisMentenReaction(S[i], E[i], P[i], k[i], 1.0)
        self.rxn3 = MichaelisMentenReaction(S[i], E[j], P[i], k[j, i], 1.0)
    ...
    def test_flux(self):
        ...
        change = range_5 * range_5 * range_5 / (range_5 + 1.0)
        self.assertTrue(jnp.allclose(dynamics_dict2[S], -change))
        self.assertTrue(jnp.allclose(dynamics_dict2[P], change))
...
class FastReactionTests(unittest.TestCase):
    ...
    def test_shapes(self):
        A, B, C, D, E, F, G = many_species("A, B, C, D, E, F, G")
        self.assertEqual(self.rxn1.shapes(), {A: (3,), B: (3,), C: (3,)})
        self.assertEqual(
            self.rxn2.shapes(),
            {
                D: (3, 3),
        ...
        self.assertEqual(self.rxn3.shapes(), {F: (), G: ()})
        self.assertEqual(self.rxn4.shapes(), dict())
    ...
    def test_index_symbols_replace(self):
        ...
        values_dict = {i: 1, j: 2}
        self.assertEqual(
            self.rxn1.index_symbols_replace(values_dict),
            FastReaction(A[1] + 2 * B[1] + C[1], A[1] + B[1] + 3 * C[1]),
        )
        self.assertEqual(
            self.rxn2.index_symbols_replace(values_dict),
    ...
    def test_flux(self):
        ...
        tensor_data = {
            ...
            B: jnp.array([1.1, 23.0, 1.4]),
            C: jnp.array([19.0, 0.3, 0.4]),
            D: jnp.array([[3.2, 0.1, 1.1], [1.4, 3.1, 3.2], [9.3, 4.3, 8.0]]),
            E: jnp.array([[2.2, 1.9, 1.3], [2.2, 1.2, 2.4], [12.0, 3.1, 2.6]]),
            F: jnp.array(1.3),
            G: jnp.array(1.7),
        }
        ...
        self.assertTrue(jnp.all(jnp.allclose(flux3[F], -jnp.array(1.3))))
        self.assertTrue(jnp.all(jnp.allclose(flux3[G], jnp.array(3 * 1.3 / 2))))
...
class ICRNTests(unittest.TestCase):
    ...
    def test_dynamics(self):
        ...
        self.assertTrue(
            jnp.allclose(normal_dynamics[B[2, 3]], 2 * 2 * tensor_data[A][1, 2])
        )
        self.assertTrue(
            jnp.allclose(
                normal_dynamics[C],
