...
class TestFastFluxF(unittest.TestCase):
    ...
    def test_standard_reactants_and_indexing(self):
        ...
        def _test_standard_reactants_and_indexing_from_rxn(
            rxn: FastReaction,
            target_standard_reactants,
            target_standard_indexing,
        ):
            try:
                standard_reactants, standard_indexing = (
                    _standard_reactants_and_indexing(rxn.reactants)
                )
                ...
                raise AssertionError(
                    f"Error testing standard reactants and indexing for {rxn}"
                ) from e
        ...
        _test_standard_reactants_and_indexing_from_rxn(
            self.rxns[3], (A[i], B[i]), (i,)
        )
        _test_standard_reactants_and_indexing_from_rxn(
            self.rxns[4], (A[i], B[i]), (i,)
        )
        _test_standard_reactants_and_indexing_from_rxn(self.rxns[5], (A,), ())
        _test_standard_reactants_and_indexing_from_rxn(
            self.rxns[6], (A[i, j], B[i, j]), (i, j)
        )
        _test_standard_reactants_and_indexing_from_rxn(
            self.rxns[7], (A[i, j], B[i, j]), (i, j)
        )
        _test_standard_reactants_and_indexing_from_rxn(
            self.rxns[8], (A[j, i],), (j, i)
        )
        _test_standard_reactants_and_indexing_from_rxn(
            self.rxns[9], (A[i, j], B[i, j]), (i, j)
        )
        _test_standard_reactants_and_indexing_from_rxn(
            self.rxns[10], (A, B, C), ()
        )
        _test_standard_reactants_and_indexing_from_rxn(
            self.rxns[11], (A[i], B[i]), (i,)
        )
    ...
    def test_build_base_einsum_str(self):
        def _test_base_einsum_str_from_rxn(rxn: FastReaction, target_str):
            try:
                ...
                self.assertEqual(
                    _build_base_einsum_str(standard_indexing), target_str
                )
            except Exception as e:
                raise AssertionError(
                    f"Error testing base einsum str for {rxn}"
                ) from e
    ...
    def test_einsum_prep(self):
        ...
        def _test_einsum_prep_from_rxn(rxn: FastReaction, target_dict):
            try:
                ...
                output = _einsum_prep(
                    rxn.reactants, rxn.products, standard_indexing
                )
                ...
                raise AssertionError(
                    f"Error testing einsum prep for {rxn}"
                ) from e

        _test_einsum_prep_from_rxn(
            self.rxns[0], {A: [(-1, "->")], B: [(-1, "->")], C: [(1, "->")]}
        )
        ...
        _test_einsum_prep_from_rxn(
            self.rxns[8], {A: [(-1, "ji->ji"), (1, "ji->ij")]}
        )

        _test_einsum_prep_from_rxn(
            self.rxns[9],
            {A: [(-1, "ij->ij"), (1, "ij->ji")], B: [(-1, "ij->ij")]},
        )

        _test_einsum_prep_from_rxn(
            self.rxns[11], {A: [(-1, "i->i")], B: [(-1, "i->i")]}
        )

    def test_get_reactant_unit_from_rxn(self):
        A, B, C, D, E = many_species("A, B, C, D, E")

        def _test_get_reactant_unit_from_rxn(
            rxn: FastReaction, state_data, target_unit
        ):
            try:
                standard_reactants, standard_indexing = (
                    _standard_reactants_and_indexing(rxn.reactants)
                )
                output = _get_reactant_unit(
                    standard_reactants, rxn.reactants, state_data
                )
                self.assertTrue(jnp.all(jnp.allclose(output, target_unit)))
            except Exception as e:
                raise AssertionError(
                    f"Error testing get reactant unit for {rxn}"
                ) from e

        _test_get_reactant_unit_from_rxn(
            self.rxns[0], {A: jnp.array(2), B: jnp.array(3)}, jnp.array(2)
        )

        _test_get_reactant_unit_from_rxn(
            self.rxns[1], {A: jnp.array(2), B: jnp.array(3)}, jnp.array(1)
        )

        _test_get_reactant_unit_from_rxn(
            self.rxns[2], {A: jnp.array(2), B: jnp.array(3)}, jnp.array(2)
        )

        _test_get_reactant_unit_from_rxn(
            self.rxns[3],
            {A: jnp.array([0, 1, 2, 3, 4]), B: jnp.array([5, 4, 3, 2, 1])},
            jnp.array([0, 1, 2, 2, 1]),
        )
        ...
        _test_get_reactant_unit_from_rxn(
            self.rxns[5], {A: jnp.array(2)}, jnp.array(2)
        )
        ...
        _test_get_reactant_unit_from_rxn(
            self.rxns[6],
            {
                ...
                B: jnp.array([[6, 5], [4, 3], [2, 1]]),
            },
            jnp.array([[0, 1], [2, 3 / 2], [1, 1 / 2]]),
        )
        ...
        _test_get_reactant_unit_from_rxn(
            ...
            jnp.array([[0, 1], [4 / 3, 1], [2 / 3, 1 / 3]]),
        )

        _test_get_reactant_unit_from_rxn(
            self.rxns[10],
            {A: jnp.array(2), B: jnp.array(3), C: jnp.array(4)},
            jnp.array(2),
        )
    ...
    def test_fast_flux_f(self):
        ...
        def test_fast_flux_f_from_rxn(
            rxn: FastReaction, state_data, target_flux: dict
        ):
            try:
                ...
                raise AssertionError(
                    f"Error testing fast flux f for {rxn}"
                ) from e

        test_fast_flux_f_from_rxn(
            self.rxns[0],
            {A: jnp.array(2), B: jnp.array(3)},
            {A: -jnp.array(2), B: -jnp.array(2), C: jnp.array(2)},
        )
        ...
        test_fast_flux_f_from_rxn(
            ...
            {
                A: jnp.array([[0, 1], [2, 3], [4, 5]]),
                B: jnp.array([[6, 5], [5, 3], [2, 1]]),
            },
            {
                A: -jnp.array([[0, 1], [2, 3 / 2], [1, 1 / 2]]),
                B: -jnp.array([[0, 2], [4, 3], [2, 1]]),
                C: jnp.array([[0, 3], [6, 9 / 2], [3, 3 / 2]]),
            },
        )
        ...
        test_fast_flux_f_from_rxn(
            ...
            {
                A: -jnp.array([[0, 1], [2, 3], [2, 1]]),
                B: -jnp.array([[0, 1], [2, 3], [2, 1]]),
                C: jnp.array([[0, 2, 2], [1, 3, 1]]),
                D: jnp.array([4, 5]),
                E: jnp.array(9),
            },
        )
        ...
        test_fast_flux_f_from_rxn(
            self.rxns[11],
            {A: jnp.array([0, 1, 2, 3, 4]), B: jnp.array([5, 4, 3, 2, 1])},
            {A: -jnp.array([0, 1, 2, 2, 1]), B: -jnp.array([0, 1, 2, 2, 1])},
        )
