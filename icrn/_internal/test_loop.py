...
from icrn._testing.checkpoint_memory import (
    backward_peak_rss_bytes,
    make_matmul_loss,
)
from icrn._testing.gpu_checkpoint_tests import (
    checkpoint_grad_matches_no_checkpoint,
    checkpoint_large_problem_fits_with_checkpoints,
    checkpoint_reduces_backward_peak_memory,
)
...
class TestLoopCheckpointMemoryCPU(unittest.TestCase):
    ...
    def test_checkpoint_length_grad_matches_no_checkpoint(self):
        ...
        with jax.default_device(cpu):
            loss_none, W = make_matmul_loss(None, n, steps, device=cpu)
            loss_ckpt, _ = make_matmul_loss(8, n, steps, device=cpu)
    ...
    def test_checkpoint_length_reduces_backward_peak_memory(self):
        ...
        peak_none = backward_peak_rss_bytes(None, n, steps)
        peak_ckpt = backward_peak_rss_bytes(checkpoint_length, n, steps)
...
class TestLoopCheckpointMemoryGPU(unittest.TestCase):
    ...
    Each test runs in a fresh subprocess with GPU JAX enabled so the parent
    test process stays on CPU.
    """

    def test_checkpoint_length_grad_matches_no_checkpoint(self):
        """Checkpointing must not change reverse-mode gradients on GPU."""
        run_gpu_test(checkpoint_grad_matches_no_checkpoint)
    ...
    def test_checkpoint_length_reduces_backward_peak_memory(self):
        ...
        run_gpu_test(checkpoint_reduces_backward_peak_memory)
    ...
    @unittest.skipUnless(
        gpu_vram_at_least(GPU_STRESS_MIN_VRAM_BYTES),
        "GPU stress test requires at least 6 GiB VRAM",
    )
    @unittest.skipUnless(
        gpu_memory_stats_supported(),
        "GPU device memory_stats peak_bytes_in_use is not available",
    ...
    def test_checkpoint_length_large_problem_fits_with_checkpoints(self):
        ...
        run_gpu_test(checkpoint_large_problem_fits_with_checkpoints)
