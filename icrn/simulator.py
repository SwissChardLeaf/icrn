...
class Experiment:
    ...
    def simulate_segments(
        ...
        return sim_state, map2(
            lambda x, y: jnp.concat([x, y]), concs_expanded, sim_hist
        )

    # want this to be jittable
    def simulate_time(
        self, concs_data, rate_constant_data, diff_data, time, dt=None, sample_num=1
    ):
        if dt is None:
            dt = self._exp_params["dt"]
