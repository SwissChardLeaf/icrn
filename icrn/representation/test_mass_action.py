...
class TestMassActionFluxF(unittest.TestCase):
    def setUp(self):
        ...
        self.good_rxns = [
            ...
            MassActionReaction(A[i, l] + B[j], A[j, m] + B[l], 1.),
    ...
    def test_get_diff_dict(self):
        ...
        self.assertEqual(_get_diff_dict_from_rxn(self.good_rxns[0]), {A: -1, B: -1, C: 1})
        self.assertEqual(_get_diff_dict_from_rxn(self.good_rxns[1]), {A[i]: -1, B[j]: -1, C[i, j]: 1})
        ...
        self.assertEqual(_get_diff_dict_from_rxn(self.good_rxns[4]), {A[i, l]: -1, B[j]: -1, A[j, m]: 1, B[l]: 1})
    ...
    def test_get_all_index_symbols(self):
        ...
        def _get_all_index_symbols_from_rxn(rxn: MassActionReaction, target_set):
            try:
                self.assertEqual(_get_all_index_symbols(rxn.reactants, rxn.products, rxn.rate_expr), target_set)
            except Exception as e:
                raise AssertionError(f"Error getting all index symbols for {rxn}") from e
    ...
    def test_product_index_symbols(self):
        ...
        def _product_index_symbols_from_rxn(rxn: MassActionReaction, target_set):
            self.assertEqual(_product_index_symbols(rxn.reactants, rxn.products, rxn.rate_expr), target_set)
    ...
    def test_base_einsum_str(self):
        def _get_base_einsum_str_from_rxn(rxn: MassActionReaction, target_str):
            try:
                product_index_symbols = _product_index_symbols(rxn.reactants, rxn.products, rxn.rate_expr)
                self.assertEqual(_get_base_einsum_str(rxn.reactants, rxn.rate_expr, product_index_symbols), target_str)
            except Exception as e:
    ...
    def test_setup_einsums(self):
        ...
        _setup_einsums_from_rxn(self.good_rxns[0], {
            A: [(-1, ",,->")],
            B: [(-1, ",,->")],
            C: [(1, ",,->")],
        })
        _setup_einsums_from_rxn(self.good_rxns[1], {
            A: [(-1, "i,i,j->i")],
            B: [(-1, "i,i,j->j")],
            C: [(1, "i,i,j->ij")],
        })
        _setup_einsums_from_rxn(self.good_rxns[2], {
            C: [(1, "->")],
        })
        _setup_einsums_from_rxn(self.good_rxns[3], {
            A: [(-1, ",i->i")],
        })
        _setup_einsums_from_rxn(self.good_rxns[4], {
            A: [(-1, ",il,j,m->il"), (1, ",il,j,m->jm")],
            B: [(-1, ",il,j,m->j"), (1, ",il,j,m->l")]
        })
        _setup_einsums_from_rxn(self.good_rxns[5], {
            A: [(-1, ",i->i")],
        })
        _setup_einsums_from_rxn(self.bad_rxns[0], {
            A: [(-1, ",i,j->i"), (1, ",i,j->j")]
        })
        _setup_einsums_from_rxn(self.bad_rxns[1], {
            A: [(-1, "k,i,j->i")],
            B: [(1, "k,i,j->j")],
        })
        _setup_einsums_from_rxn(self.bad_rxns[2], {
            A: [(1, "k,i->i")],
        })
    ...
    def test_get_tensors(self):
        ...
        def _get_tensors_from_rxn(rxn: MassActionReaction, state, non_state, target_tensors):
            try:
                product_index_symbols = _product_index_symbols(rxn.reactants, rxn.products, rxn.rate_expr)
                output = _get_tensors(rxn.reactants, rxn.rate_expr, product_index_symbols)

                output_tensor = output(state, non_state)

                if len(output_tensor) != len(target_tensors):
                    raise AssertionError(f"Error getting tensors for {rxn}, output tensor has length {len(output_tensor)}, but target tensors have length {len(target_tensors)}")

                for i in range(len(output_tensor)):
                    self.assertTrue(jnp.all(jnp.equal(output_tensor[i], target_tensors[i])))
        ...
        _get_tensors_from_rxn(
            ...
            {},
            (jnp.array(1.0), 1.0 * jnp.ones((10, 7)), 2.0 * jnp.ones((7,)), jnp.ones((7,)))
        )

        _get_tensors_from_rxn(self.good_rxns[5], 
            {
                A: 1.0 * jnp.ones((10,)),
            },
            {},
    ...
    def test_mass_action_flux_f(self):
        ...
        def _test_mass_action_flux_f_from_rxn(rxn: MassActionReaction, state, non_state, target_flux):
            try:
                ...
                raise AssertionError(f"Error testing mass action flux f for {rxn}") from e

        _test_mass_action_flux_f_from_rxn(self.good_rxns[0],
            state={
                A: jnp.array(2.0),
                B: jnp.array(3.0)
            },
        ...
        _test_mass_action_flux_f_from_rxn(self.good_rxns[1],
            state={
                A: A_data,
                B: B_data,
            },
        ...
        _test_mass_action_flux_f_from_rxn(self.good_rxns[2],
            state={},
            non_state={
                alpha: alpha_data,
            },
        ...
        _test_mass_action_flux_f_from_rxn(self.good_rxns[3],
            state={
                A: A_data,
            },
            non_state={},
        ...
        _test_mass_action_flux_f_from_rxn(self.good_rxns[4],
            state={
                A: A_data,
                B: B_data,
            },
            non_state={},
            target_flux={
                A: -jnp.einsum(",il,j,m->il", jnp.array(1.0), A_data, B_data, jnp.ones((7,))) + jnp.einsum(",il,j,m->jm", jnp.array(1.0), A_data, B_data, jnp.ones((7,))),
                B: -jnp.einsum(",il,j,m->j", jnp.array(1.0), A_data, B_data, jnp.ones((7,))) + jnp.einsum(",il,j,m->l", jnp.array(1.0), A_data, B_data, jnp.ones((7,)))
            }
        )