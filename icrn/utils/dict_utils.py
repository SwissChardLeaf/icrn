...
#                 diff_data[base] = jnp.broadcast_to(
#                     jnp.array(diff_spec.get(base, 0)), shape
#                 ).astype(dtype)

#             if batch_size is not None:
#                 init_shape = (batch_size,) + init_shape

#             concs_data[base] = jnp.broadcast_to(
#                 jnp.array(concs_spec.get(base, 0)), init_shape
#             ).astype(dtype)
...
#             rate_constant_data[base] = jnp.broadcast_to(
#                 jnp.array(rate_constant_spec.get(base, 0)), init_shape
#             ).astype(dtype)
#         else:
#             raise ValueError
